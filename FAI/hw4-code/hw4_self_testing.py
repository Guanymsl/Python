...
from src.autoencoder_activate import Autoencoder, DenoisingAutoencoder
...
def main():
    ...
    print(f"Acc from DenoisingAutoencoder: {compute_acc(y_pred_ae, y_val)}")
    ...
    print(f"Reconstruction Loss with DenoisingAutoencoder: {reconstruction_loss_ae}")
    ...
    axes[1].imshow(image_reshape(img_reconstruct_ae), cmap='gray')
