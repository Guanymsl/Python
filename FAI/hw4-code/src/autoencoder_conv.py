...
class Autoencoder(nn.Module):
    def __init__(self, input_dim: int, encoding_dim: int) -> None:
        ...
        self.encoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Flatten(),
            nn.Linear(16 * input_dim, encoding_dim),
            nn.Linear(encoding_dim, encoding_dim // 2),
            nn.ReLU()
        )
        self.decoder = nn.Sequential(
            nn.Linear(encoding_dim // 2, encoding_dim),
            nn.Linear(encoding_dim, 16 * input_dim),
            nn.ReLU(),
            nn.Unflatten(1, (16, 80, 61)),
    ...
    def transform(self, X):
        ...
        self.eval()
        with torch.no_grad():
            encoded = self.encoder(X.view(X.size(0), 1, 80, 61))
    ...
    def reconstruct(self, X):
        ...
        self.eval()
        with torch.no_grad():
            reconstructed = self(X.view(1, 1, 80, 61))
