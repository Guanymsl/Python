...
def encode_labels(y: np.ndarray) -> np.ndarray:
    ...
    return np.array([labels[label] for label in y])
...
class LinearModel:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        if self.model_type == "logistic":
            ...
            y_predict = np.eye(n_classes)[y]
            for _ in range(self.iterations):
                gradients = self._compute_gradients(X, y_predict)
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        if self.model_type == "linear":
            ...
            # TODO: 2%
            return np.argmax(self._softmax(X.dot(self.weights)), axis=1)

    def _compute_gradients(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        if self.model_type == "linear":
            # TODO: 3%
            return 2 * (X.dot(self.weights) - y).T.dot(X) / len(y)
        elif self.model_type == "logistic":
            # TODO: 3%
            N = len(y)
            gradients = np.zeros((X.shape[1], y.shape[1]))
            for n in range(N):
                gradients += self._softmax((-y[n] * X[n].dot(self.weights)).reshape(1, -1)) * (-X[n].reshape(-1, 1).dot(y[n].reshape(1, -1)))
            return gradients / N
...
class DecisionTree:
    ...
    def _gini_index(self, left_y: np.ndarray, right_y: np.ndarray) -> float:
        ...
        def gini(dataset):
            s = len(dataset)
            return 1.0 - sum((np.sum(dataset == c) / s) ** 2 for c in np.unique(dataset))

        return (Sl * gini(left_y) + Sr * gini(right_y)) / (Sl + Sr)
    ...
    def _mse(self, left_y: np.ndarray, right_y: np.ndarray) -> float:
        ...
        def mse(dataset):
            return np.mean((dataset - np.mean(dataset)) ** 2)

        return (Sl * mse(left_y) + Sr * mse(right_y)) / (Sl + Sr)
...
class RandomForest:
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        if self.model_type == "classifier":
            return np.apply_along_axis(lambda x: Counter(x).most_common(1)[0][0], axis = 0, arr = predicts)
        else:
